...
export const useUserStore = defineStore('user', () => {
  ...
  const userDisplayName = computed(() => 
    currentUser.value?.getDisplayName() || '未登录用户'
  )
...
export const userApi = {
  ...
  login: (credentials: { email: string; password: string }) =>
    httpClient.post<IUser>('/auth/login', credentials),
   ...
   */
  getCurrentUser: () =>
    httpClient.get<IUser>('/user/profile'),
   ...
   * @param updates 更新数据
   */
  updateUser: (id: string, updates: Partial<IUser>) =>
    httpClient.put<IUser>(`/user/${id}`, updates),

  /**
   * 创建用户
   * @param userData 用户数据
   */
  createUser: (userData: ICreateUserRequest) =>
    httpClient.post<IUser>('/user', userData),
...
const isCurrentUser = computed(() => 
  !props.userId || props.userId === userStore.currentUser?.id
)
