...
export class PanelConfigRepository {
  ...
  public async findById(id: number): Promise<PanelConfig | null> {
    try {
      const config = await this.db.get<PanelConfig>(
        'SELECT * FROM panel_configs WHERE id = ?',
        [id]
      );
      return config || null;
    } catch (error) {
  ...
  public async update(id: number, updateData: UpdatePanelConfigData): Promise<PanelConfig | null> {
    try {
      ...
      await this.db.run(
        `UPDATE panel_configs SET ${setParts.join(', ')} WHERE id = ?`,
        values
      );
  ...
  public async delete(id: number): Promise<boolean> {
    try {
      const result = await this.db.run(
        'UPDATE panel_configs SET is_active = 0 WHERE id = ?',
        [id]
      );
