...
export class UserRepository {
	...
	public async findByUserId(userId: string): Promise<User | null> {
		try {
			const user = await this.db.get<User>('SELECT * FROM users WHERE user_id = ?', [userId])
			return user || null
		} catch (error) {
	...
	public async update(userId: string, updateData: UpdateUserData): Promise<User | null> {
		try {
			...
			await this.db.run(`UPDATE users SET ${setParts.join(', ')} WHERE user_id = ?`, values)
	...
	public async delete(userId: string): Promise<boolean> {
		try {
			const result = await this.db.run('UPDATE users SET is_active = 0 WHERE user_id = ?', [userId])
