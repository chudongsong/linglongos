...
export class ApiLogRepository {
	...
	public async findById(id: number): Promise<ApiLog | null> {
		try {
			const log = await this.db.get<ApiLog>('SELECT * FROM api_logs WHERE id = ?', [id])
			return log || null
		} catch (error) {
	...
	public async deleteOldLogs(retentionDays: number = 30): Promise<number> {
		try {
			const result = await this.db.run('DELETE FROM api_logs WHERE created_at < datetime("now", "-" || ? || " days")', [
				retentionDays,
			])
