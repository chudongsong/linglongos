...
export class OnePanelDetector implements DetectionStrategy {
	async detect(endpoint: string, headers: Record<string, string> = {}): Promise<PanelDetectionResult> {
		try {
			...
			const detectionEndpoints = ['/api/v1/auth/captcha', '/api/v1/auth/login', '/api/v1/system/info', '/api/v1']
...
export class BaotaDetector implements DetectionStrategy {
	async detect(endpoint: string, headers: Record<string, string> = {}): Promise<PanelDetectionResult> {
		try {
			...
			// 尝试通过检查常见端点来检测宝塔
			const detectionEndpoints = ['/login', '/api', '/bt', '/']
			...
			for (const path of detectionEndpoints) {
				try {
					...
					if (response.data && typeof response.data === 'object') {
						if ('status' in response.data || 'msg' in response.data || 'data' in response.data) {
...
export class PanelDetectionService {
	private detectors: DetectionStrategy[] = [new OnePanelDetector(), new BaotaDetector()]
