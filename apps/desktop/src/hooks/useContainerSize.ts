...
export function useContainerSize(initialWidth = 0, initialHeight = 0) {
  ...
  const [containerWidth, setContainerWidth] = useState<number>(typeof window !== 'undefined' ? window.innerWidth : initialWidth)
  const [containerHeight, setContainerHeight] = useState<number>(typeof window !== 'undefined' ? window.innerHeight : initialHeight)
