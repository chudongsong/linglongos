...
export const useAuthStore = defineStore('auth', () => {
  ...
  const userDisplayName = computed(() => 
    currentUser.value?.username || currentUser.value?.email || '未知用户'
  )
   ...
   * @returns 模拟响应
   */
  const mockLoginApi = async (credentials: ILoginRequest): Promise<{ success: boolean; data: ILoginResponse; message: string }> => {
    // 模拟网络延迟
    await new Promise(resolve => setTimeout(resolve, 1000))
    
    // 模拟用户验证
    if (credentials.email === 'admin@linglongos.com' && credentials.password === 'admin123') {
