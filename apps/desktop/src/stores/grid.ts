...
export const useGridStore = defineStore('grid', () => {
   ...
   * 获取网格项目数组
   */
  const gridItemsArray = computed(() => 
    Array.from(gridItems.value.values())
  )
  ...
  const visibleGridItems = computed(() =>
    gridItemsArray.value.filter(item => item.general.visible)
  )
  ...
  const validatePosition = (position: GridPosition, excludeItemId?: string): PositionValidation => {
    // 边界检查
    if (position.x < 0 || position.x >= currentConfig.value.columns ||
        position.y < 0 || position.y >= currentConfig.value.rows) {
      return {
        isValid: false,
        reason: '位置超出网格边界'
      }
    ...
    for (const item of gridItems.value.values()) {
      if (item.id !== excludeItemId && 
          item.general.visible && 
          getPositionKey(item.position) === positionKey) {
        return {
          isValid: false,
          reason: '位置已被占用',
   ...
   * 批量移动项目
   */
  const moveMultipleItems = (moves: Array<{ id: string; position: GridPosition }>, options: GridOperationOptions = {}): boolean => {
    const { autoSave = true } = options
