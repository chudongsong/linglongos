...
function createProgressUpdater(elements: { fill: HTMLElement | null; text: HTMLElement | null }) {
  ...
  return (value: number, _label?: string) => {
    const v = Math.max(0, Math.min(100, Math.round(value)))
    if (elements.fill) elements.fill.style.width = `${v}%`
    if (elements.text) elements.text.textContent = `${BASE_LABEL} ${v}%`
  }
}
...
class LoadingManager {
  ...
  private updateComposite() {
    ...
    const sum = styles * this.weights.styles + dom * this.weights.dom + config * this.weights.config + preload * this.weights.preload
