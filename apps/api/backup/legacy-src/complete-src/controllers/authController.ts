...
import { securityLogger } from '../middlewares/loggerMiddleware'
...
const googleAuthBind: Middleware = withResponse(async (ctx) => {
	logger.info('Google Auth Bind Request', {
		ip: ctx.ip,
	...
	securityLogger.logAuthEvent('login_success', undefined, ctx.ip, {
		action: 'google_auth_bind_generated',
		bindId: bindId.substring(0, 8) + '...', // 只记录部分 ID 以保护隐私
	})

	logger.debug('Google Auth Bind Generated', {
...
const googleAuthVerify: Middleware = withResponse(async (ctx) => {
	...
	logger.info('Google Auth Verify Attempt', {
		ip,
		userAgent,
		hasToken: !!token,
	})
	...
	if (!token) {
		securityLogger.logAuthEvent('login_failed', undefined, ip, {
	...
	// 记录成功的认证
	securityLogger.logAuthEvent('login_success', undefined, ip, {
		sessionId: session.id.substring(0, 8) + '...',
		bindId: bindId.substring(0, 8) + '...',
		expiresAt: new Date(session.expiresAt).toISOString(),
	})

	logger.info('Google Auth Verification Successful', {
		sessionId: session.id.substring(0, 8) + '...',
		bindId: bindId.substring(0, 8) + '...',
		ip,
