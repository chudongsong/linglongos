import { z } from 'zod'
import { bindPanelKeySchema, proxyRequestSchema } from '../controllers/proxyController.js'
import { btProxySchema } from '../btpanel/index.js'
import { zodToJsonSchema } from 'zod-to-json-schema'
import { proxyRouteDocs } from '../controllers/proxyController.js'
import { btpanelRouteDocs } from '../btpanel/index.js'
import { authRouteDocs } from '../controllers/authController.js'
/**
 * OpenAPI 文档生成模块
 *
...
/**
 * RouteDocMeta - 路由文档元数据类型
 *
 * 描述每个接口的基本信息与请求体验证模式，供自动聚合扫描使用。
 */
export type RouteDocMeta = {
  tag: string
  /** HTTP 方法，使用小写键以匹配 OpenAPI paths 字段 */
  method: 'get' | 'post' | 'put' | 'delete' | 'patch'
  /** 路径（包含 /api/v1 的功能前缀，如 /proxy/xxx、/auth/xxx） */
  path: string
  /** 可选的 Zod 请求体验证模式，存在时自动转换为 OpenAPI Schema */
  requestSchema?: z.ZodTypeAny
  /** 可选的 Zod 查询参数模式（用于 GET 等），存在时自动转换为 OpenAPI parameters */
...
export function getOpenApiSpec(): OpenAPIObject {
  const spec: OpenAPIObject = {
    ...
    info: {
      ...
      description:
        '基于 Koa 的接口服务，支持 Google Auth 认证与多面板代理。该文档由代码生成，确保与路由实现一致。',
      version: '1.0.0',
    ...
    paths: {
        ...
        get: {
          ...
          responses: {
            '200': {
              description: '成功',
              content: {
                'application/json': { schema: apiResponse({ type: 'object', properties: { qrCodeUrl: { type: 'string' }, secret: { type: 'string' } } }) },
              },
            },
          },
        },
      ...
      '/auth/google-auth-verify': {
        post: {
          ...
          responses: {
            '200': { description: '认证成功', content: { 'application/json': { schema: apiResponse({ type: 'null' }) } } },
        ...
        get: {
          ...
          responses: {
            '200': {
              ...
              content: {
                'application/json': {
                  schema: apiResponse({ type: 'object', properties: { isConfigured: { type: 'boolean' }, configuredAt: { type: 'number', nullable: true } } }),
                },
              },
            },
      ...
      '/proxy/bind-panel-key': {
        post: {
          ...
          // 请求体将由自动聚合扫描注入
          responses: {
            '200': { description: '绑定成功', content: { 'application/json': { schema: apiResponse({ type: 'null' }) } } },
      ...
      '/proxy/request': {
        get: {
          ...
          responses: {
            '200': { description: '成功', content: { 'application/json': { schema: apiResponse({ type: 'object' }) } } },
            '400': { description: '参数无效', content: { 'application/json': { schema: apiError() } } },
    ...
    components: {
      securitySchemes: {
        CookieAuth: {
          type: 'apiKey', in: 'cookie', name: 'll_session', description: '会话 Cookie 鉴权',
        },
      },
      schemas: {
