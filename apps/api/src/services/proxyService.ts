...
import { attachBtAuth } from '../middlewares/btMiddleware.js'
	...
	try {
		...
		if (axios.isAxiosError(error)) {
			...
			if (errorCode === 'CERT_UNTRUSTED' || 
				...
				if (!ignoreSslErrors) {
					throw new Error(`SSL证书验证失败: ${errorMessage}. 请检查目标服务器的SSL证书配置，或在请求中设置 ignoreSslErrors: true 来跳过证书验证。`)
				}
			}
